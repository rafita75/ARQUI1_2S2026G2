...
11. [Estado del proyecto y trabajo pendiente](#11-estado-del-proyecto-y-trabajo-pendiente)
...
| (colección ARM64) | `Arm64Schema` | `maximo`, `minimo`, `promedio`, `total_datos`, `tiempo_ms` |
| `comandos` (modelo definido, integración pendiente) | `ComandoSchema` | `actuador`, `accion`, `origen`, `timestamp` |
...
- La Raspberry Pi publica un payload JSON combinado con todas las lecturas y
  el estado de los actuadores en `grupo2/edificio/sensores`.
- El dashboard publica comandos en `grupo2/edificio/comandos/<dispositivo>`
  (actualmente `puerta` y `luces` están conectados en el backend).
- El estado global se transmite embebido en el mismo payload de sensores.

Listado completo de topics, comparación contra los topics mínimos del
enunciado y pendientes: [`docs/mqtt_topics.md`](mqtt_topics.md).
...
- LED verde (GPIO13) = `NORMAL`, LED amarillo (GPIO19) = `ADVERTENCIA`.
- Reglas de transición evaluadas en el bucle principal:
  ...
  - `ADVERTENCIA` si el ventilador está encendido (temperatura alta).
...
- **Controles remotos**: puerta, luces (on/off/auto), ventilador y
  seguridad (silenciar/reset) — los dos últimos publican comandos MQTT pero
  el backend aún no está suscrito a esos topics (ver sección 11).
...
- **Payload MQTT combinado** en lugar de un topic por sensor: simplifica la
  sincronización en el dashboard (una sola suscripción, un solo `render`),
  a costa de no cumplir literalmente la granularidad de topics mínimos que
  pide el enunciado (documentado como pendiente).
...
## 11. Estado del proyecto y trabajo pendiente

Checklist frente a los requisitos obligatorios del enunciado, para guiar el
trabajo restante (ver también el análisis detallado ya discutido con el
equipo):
...
Estos pendientes requieren modificar `backend/main.py` y no se aplicaron
directamente porque el equipo no tiene acceso físico a la Raspberry Pi para
probarlos en este momento; quedan documentados aquí para implementarlos y
validarlos en la siguiente sesión con hardware disponible.
